...
CD8toCD4score <- function(TCR) {
    ...
    for (i in seq_len(nrow(TCR))) {
        aascore <- 0
        vscore <- values_gene[which(TCR$v[i] %in% names(values_gene))]
        jscore <- values_gene[which(TCR$j[i] %in% names(values_gene))]
        lscore <- values_length[which(nchar(as.character(TCR$Var1[i])) %in% names(values_length))]
        for (j in seq_len(nchar(as.character(TCR$Var1[i])))) {
            char <- as.character(TCR$Var1[i])
    ...
    if (length(lscore) == 0) {
        score$score[i] <- (vscore + jscore + aascore)/3
    } else {     
        score$score[i] <- (vscore + jscore + lscore + aascore)/4
    }
    }
    return(score)
}
