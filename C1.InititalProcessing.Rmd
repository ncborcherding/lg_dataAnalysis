...
for (i in seq_along(file_list)){
    tmp <-  Read10X(paste0("./data/", file_list[i]))
    colnames(tmp) <- paste(file_list[i], colnames(tmp), sep = '_')
    tmp <- CreateSeuratObject(tmp)
    x <- which(rownames(tmp@assays$RNA) %in% mito)
    rownames(tmp@assays$RNA)[x] <- paste0("MT-", rownames(tmp@assays$RNA)[x])
    ...
    dev.off()
    tmp <- subset(tmp, subset = mito.genes < 10 & nFeature_RNA < 3000)
...
rm(list)
anchors <- IntegrateData(anchorset = anchors, normalization.method = "SCT", 
    verbose = FALSE)
saveRDS(anchors, "./data/integrated_anchors.rds")
```
...
for (i in c(10,15,20,25,30,35,40)) {
    ...
    integrated2 <- ScaleData(object = anchors, verbose = FALSE)
