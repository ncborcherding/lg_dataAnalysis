...
graph <- graph_from_adjacency_matrix(, mode = "undirected")
graph <- simplify(graph)
...
coldef <- viridis::viridis_pal(option = "H")(length(table(walktrap$membership)))

x <- components(graph, mode = "weak")

plot.igraph(graph,layout=layout.fruchterman.reingold,vertex.color=coldef[walktrap$membership],
